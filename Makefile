...
.PHONY: docker-build docker-up docker-down docker-logs docker-deploy init-env run-freq run-brale
...
help:
	...
	@echo "  make run-freq - 启动 freqtrade 容器"
	@echo "  make run-brale - 构建并启动 brale 容器（会自动确保 freqtrade 先运行）"
